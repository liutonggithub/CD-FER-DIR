...
def conv2d(input_map, num_output_channels, size_kernel=5, stride=2, name='conv2d'):
    with tf.variable_scope(name):
        ...
        # input_map：[batch, in_height, in_width, in_channels]分别表示 批次数，图像高度，宽度，输入通道数
        # 卷积核。格式要求为[filter_height, filter_width, in_channels, out_channels].
...
#    添加了with tf.variable_scope("encoder") as scope:if reuse_variables:tf.get_variable_scope().reuse_variables()
...
# 添加了with tf.variable_scope(tf.get_variable_scope(), reuse=False):
...
def concat_label(x, label, duplicate=1):
    x_shape = x.get_shape().as_list()  # 只能用于tensor来返回shape，但是是一个元组(2,3)，需要通过as_list()的操作转换成list[2,3]
    if duplicate < 1:
        return x
    # duplicate the label to enhance its effect, does it really affect the result?
    #  tile() 函数，就是将原矩阵横向、纵向地复制
    label = tf.tile(label, [1, duplicate])  # (49,7),(49,5)->(49,15)
    label_shape = label.get_shape().as_list()
    if len(x_shape) == 2:
        return tf.concat([x, label], 1)
    elif len(x_shape) == 4:
        label = tf.reshape(label, [x_shape[0], 1, 1, label_shape[-1]])  # (49,1,1,7)
...
def save_batch_images(
    ...
    # transform the pixcel value to 0~1 batch_images值为-1到1
    ...
    if size_frame is None:
        auto_size = int(np.ceil(np.sqrt(images.shape[0])))  # 计算大于等于该值的最小整数
    ...
    for ind, image in enumerate(images):
        ind_col = ind % size_frame[1]  # 计算除法的余数
        ind_row = ind // size_frame[1]  # 表示整数除法
